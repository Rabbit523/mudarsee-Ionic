...
import { TutorContactInformationPage } from '../tutor-contact-information/tutor-contact-information';

@Component({
  selector: 'page-tutor-contact-details',
  templateUrl: 'tutor-contact-details.html',
  providers: [HttpService, GlobalMethods, ServiceName, ValidationService]
})
export class TutorContactDetailsPage {
  userData: any;
  countries: any;
  contactForm: any;
    ...
    this.httpService.httpServicePost(this.serviceName.updateStudentContactDetails, params).subscribe(
      data => {
        ...
        if (data[1].response.status == 1) {
          ...
          this.navCtrl.setRoot(TutorContactInformationPage);
        } else {
          this.globalMethods.showToast(data[1].response.message);
        }
      },
